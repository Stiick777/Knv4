...
let handler = async (m, { conn, args, usedPrefix, command, DevMode }) => {
    ...
    conn.reply(m.chat, `⚠️ Ya has iniciado una apuesta recientemente, espera *⏱ ${tiempoRestante}* para apostar nuevamente`, m, rcanal)
    return
  }
  ...
  cooldowns[m.sender] = Date.now()
        ...
        if (args.length < 1) return conn.reply(m.chat, '✳️ Ingresa la cantidad de ' + `*⭐ Estrellas*` + ' que deseas aportar contra' + ` *Ai Hoshino*` + `\n\n` + '`Ejemplo:`\n' + `> *${usedPrefix + command}* 100`, m, rcanal)
