...
    // Aplicamos la misma lógica de let str del primer handler
    let str;
    if (m.mentionedJid.length > 0) {
        str = `\`${name2}\` está tímido por \`${name || who}\` ( ⚆ _ ⚆ ).`;
    } else if (m.quoted) {
        str = `\`${name2}\` está tímido por \`${name || who}\`.`;
    } else {
        str = `\`${name2}\` está tímido consigo mismo ( ⚆ _ ⚆ ).`.trim();
    }
